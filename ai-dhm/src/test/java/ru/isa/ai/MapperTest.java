...
public class MapperTest  extends TestCase {
    ...
    private void SimpleMapperTest() throws Exception{
       ...
       ArrayList<ArrayList<Vector2D>> colsMap;
        colsMap = mapper.mapAll(new int[]{3, 1}, new int[]{1, 1}, 1);
       ...
       colsMap = mapper.mapAll(new int[]{5, 3}, new int[]{2, 2}, 1);
        ...
        colsMap = mapper.mapAll(new int[]{2, 2}, new int[]{2, 2}, 1);
        assertTrue(colsMap.size()==4);
        ...
        try {
            colsMap = mapper.mapAll(new int[]{2, 2}, new int[]{4, 4}, 1);
            assertTrue(false);
        }
        catch (Exception e)
        {
        ...
        colsMap = mapper.mapAll(new int[]{16, 1}, new int[]{4, 1}, 2);
        assertTrue(colsMap.size()==4);
    ...
    private void mapOneTest() {
        ...
        colMap = mapper.mapOne(new int[]{1, 1}, new int[]{1, 1}, 1);
        assertTrue(colMap.get(0).getX()==0 && colMap.get(0).getY()==0);

        colMap = mapper.mapOne(new int[]{1, 1}, new int[]{1, 1}, 10);
        assertTrue(colMap.get(0).getX()==0 && colMap.get(0).getY()==0);

        colMap = mapper.mapOne(new int[]{3, 1}, new int[]{1, 1},1);
        assertTrue(colMap.get(0).getX()==0 && colMap.get(0).getY()==0 &&
                   colMap.get(1).getX()==1 && colMap.get(1).getY()==0 &&
                   colMap.get(2).getX()==2 && colMap.get(2).getY()==0);

        colMap = mapper.mapOne(new int[]{3, 1}, new int[]{2, 1}, 1);
        assertTrue(colMap.get(0).getX()==1 && colMap.get(0).getY()==0 &&
                   colMap.get(1).getX()==2 && colMap.get(1).getY()==0);

        colMap = mapper.mapOne(new int[]{5, 3}, new int[]{2, 2},2);
