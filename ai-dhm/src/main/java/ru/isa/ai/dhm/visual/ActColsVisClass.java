...
import java.util.*;
    ...
    public class ActColsVisClass extends JPanel {
        ...
        private BitMatrix getDataForRegion(Region r){
            ...
            BitVector v = new BitVector(dims[0]*dims[1]);
            v = r.getActiveColumns();
            for (int i = 0; i < dims[0]; i++){
                for (int j = 0 ; j < dims[1]; j++){
                    m.put(i,j,v.get(i*dims[0]+j));
                }
            }
            return m;
        }

        private void getDataDown(Object obj){
            if (obj instanceof Region) {
                int[] dims = new int[2];
                dims = ((Region)obj).getDimensions();
                dataDown = new BitMatrix(dims[0], dims[1]);
                dataDown= getDataForRegion((Region) obj);
            }
            else //BitMatrix
                {
                    dataDown = new BitMatrix(((BitMatrix) obj).columns(),((BitMatrix) obj).rows());
                    dataDown = (BitMatrix)obj;
                }
        }
        ...
        public ActColsVisClass(Region up, Object down) {
            int[] dims = new int[2];
            dims = up.getDimensions();
            dataUp = new BitMatrix(dims[0], dims[1]);
