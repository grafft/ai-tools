...
import ru.isa.ai.psyta.service.linguistic.SimpleCommonTextAnalizer;
...
public class HomePage extends ACMPage {
   ...
   @SpringBean
   private SimpleCommonTextAnalizer analizer;
         ...
         if (keywordFiles.size() > 0) {
            try {
               ...
               for (File keywordFile : keywordFiles) {
                  List<String> keywords = SimpleCommonTextAnalizer.loadProtoSignsFromFile(keywordFile.getAbsolutePath());
               ...
               // analyzing
               analizer.open();
               analizer.setKeywords(allKeyWords);
               analizer.setProgressor(new SNCProgressor() {
                  @Override
                  public void markProgress(double part) {
                     AnalyzeWorker.this.session.setProgress(part);
                  }
               });
               analizer.analize(primer);
               Map<String, Integer> results = analizer.getResult();
               analizer.resetResult();
               analizer.close();
               ...
               session.info("Файл " + file.getName() + " проанализирован за " + FormatUtils.getFormattedIntervalFromCurrent(currentTime));
